...
st.title("ChessMaster AI: Advanced Chess Engine")
...
def execute_move(move: str) -> str:
    try:
        ...
        if st.session_state.board.is_checkmate():
            winner = 'GrandMaster (White)' if st.session_state.board.turn == chess.BLACK else 'DeepThinker (Black)'
...
if st.session_state.openai_api_key:
    try:
        ...
        agent_white = ConversableAgent(
            name="GrandMaster",  
            system_message="You are a professional chess player and you play as GrandMaster (White). "
            "First call available_moves() first, to get list of legal available moves. "
            "Then call execute_move(move) to make a move.",
            llm_config={"config_list": grandmaster_white_config, "cache_seed": None},
        )

        agent_black = ConversableAgent(
            name="DeepThinker",  
            system_message="You are a professional chess player and you play as DeepThinker (Black). "
            "First call available_moves() first, to get list of legal available moves. "
            "Then call execute_move(move) to make a move.",
            llm_config={"config_list": deepthinker_black_config, "cache_seed": None},
        )
...
- **GrandMaster (White)**: A GPT-4o-mini powered chess player controlling white pieces
- **DeepThinker (Black)**: A GPT-4o-mini powered chess player controlling black pieces
